...
namespace BPServer.SignalR
{
    public class SignalRClient
    {
        ...
        public SignalRClient(SignalROptions options, IMessageBus messageBus, ILogger logger)
        {
            bus = messageBus ?? throw new ArgumentNullException(nameof(messageBus));
        ...
        private Task ConnectionReconnected(string connectionId)
        {
            ...
            log.Verbose("SignalR Reconnected");
        ...
        private Task ConnectionReconnecting(Exception error)
        {
            ...
            log.Verbose("SignalR Reconnecting");
            return Task.CompletedTask;
        }
        ...
        public async Task<bool> ConnectWithRetryAsync(HubConnection connection, CancellationToken token)
        {
            ...
            while (true)
            {
                ...
                {
                    log.Error($"SignalR ex: '{e.ToString()}'");
                    // Failed to connect, trying again in 5000 ms.
                    Debug.Assert(connection.State == HubConnectionState.Disconnected);
                    log.Verbose("SignalR Trying to connect");
